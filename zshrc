...
prompt_bar_left_self="%{%F{green}%}(%{%B%}%n%{%b%f%}%{%F{cyan}%}@%{%F{green}%}%{%B%}%m%{%b%})%{%f%}"
my_prompt_left="%F{green}%{%B%}%n%{%b%f%}%{%F{cyan}%}@%{%F{green}%}%{%B%}%m%{%b%}~%(1j,(%j),)%{%B%F{green}%}%#%{%b%}%{%f%} "
...
prompt_bar_right="[%{%B%F{red}%}%d%{%f%k%b%}]"
